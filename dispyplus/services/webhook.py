# Dispyplus: Webhook関連の機能を提供するモジュール
import discord
import aiohttp
    ...
    *,
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            ...
            # embeds と embed の処理 (discord.py Webhook.send は embeds を取る)
            actual_embeds: List[discord.Embed] = []
            if embeds:
                actual_embeds.extend(embeds)
            if embed and embed not in actual_embeds : # embed が単独で指定され、embeds に含まれていない場合
                actual_embeds.append(embed)


            sent_message = await webhook.send(
                content=content,
                username=username or bot.user.name if bot.user else None,
                avatar_url=avatar_url or bot.user.display_avatar.url if bot.user else None,
                tts=tts,
                file=final_file,
                files=final_files,
                embeds=actual_embeds if actual_embeds else None, # 空リストではなくNoneを渡す
                allowed_mentions=allowed_mentions or bot.allowed_mentions,
                wait=wait
            )
            return sent_message
